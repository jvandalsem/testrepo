...
## Your name: John Van Dalsem
## The names of anyone you worked with on this project: Trevor Woods and Phill Brown
...
def get_user_tweets(user):
	...
	if user in CACHE_DICTION:
		...
		user_data = api.user_timeline(screen_name = handle, count = 20)
		CACHE_DICTION[handle] = user_data
...
for a in umich_tweets:
	user_data = (a['user']['id_str'], a['user']['screen_name'], a['user']['favourites_count'], a['user']['description'])
	try:
		cur.execute('INSERT INTO Users (user_id, screen_name, num_favs, description) VALUES (?, ?, ?, ?)', user_data)
	except:
		pass
	for b in a['entities']['user_mentions']:
		m_user = api.get_user(b['id'])
		m_data = (m_user['id_str'], m_user['screen_name'], m_user['favourites_count'], m_user['description'])
		try:
			cur.execute('INSERT INTO Users (user_id, screen_name, num_favs, description) VALUES (?, ?, ?, ?)', m_data)
		except:
			pass
...
cur.execute('CREATE TABLE Tweets (tweet_id TEXT PRIMARY KEY, tweets TEXT, user_posted TEXT, time_posted DATETIME, retweets INTEGER, FOREIGN KEY(user_posted) REFERENCES Users(user_id))')
for a in umich_tweets:
	tweets_data = (a['id_str'], a['text'], a['user']['id_str'], a['created_at'], a['retweet_count'])
	try:
		cur.execute('INSERT INTO Tweets (tweet_id, tweets, user_posted, time_posted, retweets) VALUES (?, ?, ?, ?, ?)', tweets_data)
	except:
		pass
...
user_table = cur.execute('SELECT * FROM Users')
...
sn_data = cur.execute('SELECT screen_name FROM Users')
screen_names = []
for a in user_table:
	screen_names.append(a[0])
...
retweets = []
rt_10 = cur.execute('SELECT * FROM Tweets where retweets > 10')
for a in rt_10:
	retweets.append(a)
...
favorites = []
fav_500_users = cur.execute('SELECT description FROM Users where num_favs > 500')
for a in fav_500_users:
	favorites.append(a[0])

# Make a query using an INNER JOIN to get a list of tuples with 2
# elements in each tuple: the user screenname and the text of the
# tweet. Save the resulting list of tuples in a variable called joined_data2.
joined_data = list(cur.execute('SELECT Users.screen_name, Tweets.tweets FROM Users INNER JOIN Tweets ON Users.user_id == Tweets.user_posted'))
...
joined_data2 = list(cur.execute('SELECT Users.screen_name, Tweets.tweets FROM Users INNER JOIN Tweets ON Users.user_id == Tweets.user_posted ORDER BY Tweets.retweets DESC'))
